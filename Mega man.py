...
# Функция обработки уровня и занесения спрайтов в группы
        ...
        for event in pygame.event.get():
            ...
            if event.type == BLINKING_RECT:
                ...
                for button in range(joystick.get_numbuttons()):
                    if button == 7 and joystick.get_button(button):
                        return
        if not show_help:
...
# Выбор уровня
def stage_select():
    font = pygame.font.Font('data/Fonts/PerfectDOSVGA.ttf', 50)
                ...
                for hat in range(joystick.get_numhats()):
                    ...
                    if hats[0] == -1:
                        if not boss_chosen:
                            choice -= 1
                            MENU_SELECT.play(0)
            elif event.type == pygame.JOYBUTTONDOWN and not boss_chosen:
                for button in range(joystick.get_numbuttons()):
                    if button == 7 and joystick.get_button(button):
                        boss_chosen = True
                        y_vel -= 20
...
def continue_menu():
    main_font = pygame.font.Font('data/Fonts/PerfectDOSVGA.ttf', 70)
    font = pygame.font.Font('data/Fonts/PerfectDOSVGA.ttf', 50)
    buttons = ['CONTINUE', 'STAGE SELECT']
    main_text = main_font.render("GAME OVER", True, (255, 255, 255))
    score = font.render(f'Очков: {SCORE}', True, (255, 255, 255))
...
def victory_screen():
    ...
    font = pygame.font.Font('data/Fonts/PerfectDOSVGA.ttf', 50)
    font_score = pygame.font.Font('data/Fonts/PerfectDOSVGA.ttf', 40)
    ...
    score = font_score.render(f'Очков: {SCORE}', True, (255, 255, 255))
    buttons_text = [font.render(i, True, (255, 255, 255)) for i in buttons]
...
def print_score():
    font = pygame.font.Font('data/Fonts/PerfectDOSVGA.ttf', 70)
        ...
        if self.active:
            ...
            if distance <= 400:
                if self.on_ground and not self.damage_was:
                    if self.on_ground_count >= self.ready_jump:
                        self.jump()
...
class RollingCutter(pygame.sprite.Sprite):
    ...
    def get_speed(self):
        speed_x = self.direction[0] // 50
        speed_y = self.direction[1] // 50
        if abs(speed_x) > 10:
            speed_x = 10 * minus_or_plus(speed_x)
        if abs(speed_y) > 10:
            speed_y = 10 * minus_or_plus(speed_y)
        if abs(speed_x) < 6:
            speed_x = 6 * minus_or_plus(speed_x)
...
def new_game(level):
    global ALL_SPRITES, BULLETS, MOBS, PLATFORMS, STAIRS, SPIKES, ENEMY_BULLETS, \
        BONUS_BALLS, HEAL_CUPSULES, SCORE, BOSSES, BOSS_IS_DESTROYED
    ALL_SPRITES = Group()
    BULLETS = Group()
    ENEMY_BULLETS = Group()
    MOBS = Group()
